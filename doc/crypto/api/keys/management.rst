...
*   `psa_generate_key()` and `psa_generate_key_ext()` create a key from randomly generated data.
*   `psa_key_derivation_output_key()` and `psa_key_derivation_output_key_ext()` create a key from data generated by a pseudorandom derivation process. See :secref:`kdf`.
...
.. macro:: PSA_KEY_PRODUCTION_PARAMETERS_INIT
    ...
    Calling `psa_generate_key_ext()` or `psa_key_derivation_output_key_ext()` with :code:`params == PSA_KEY_PRODUCTION_PARAMETERS_INIT` and ``params_data_length == 0`` is equivalent to calling `psa_generate_key()` or `psa_key_derivation_output_key()`
    respectively.
