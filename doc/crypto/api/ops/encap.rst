...
.. function:: psa_encapsulate
    ...
    The ``encapsulation`` should be sent to the counterparty with the encrypted message to permit decapsulation. 
...
.. function:: psa_decapsulate
    ...
    .. summary::
       Uses a private key to decapsulate a shared secret from encapsulated data received from a counter party. Depending on the protocol, this secret may be suitable to be used directly, or may need to be passed to a KDF to derive encryption and authentication keys. 
    ...
    The size of ``output_key`` is always the bit-size of the shared secret, rounded up to a whole number of bytes. The size is of the shared secret is dependent on the encapsulation algorithm and cipher suite.
