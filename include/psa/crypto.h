...
/**
 * \brief Sign a message with a private key using a supplied context. For
 *        hash-and-sign algorithms, this includes the hashing step.
 *
 * \note To perform a multi-part hash-and-sign signature algorithm, first use
 *       a multi-part hash operation and then pass the resulting hash to
 *       psa_sign_hash(). PSA_ALG_GET_HASH(\p alg) can be used to determine the
 ...
 * \param[in]  input            The input message to sign.
 * \param[in]  input_length     Size of the \p input buffer in bytes.
 * \param[in]  context          The context to use for this signature.
 * \param[in]  context_length   Size of the \p context buffer in bytes.
 * \param[out] signature        Buffer where the signature is to be written.
 * \param[in]  signature_size   Size of the \p signature buffer in bytes. This
 *                              must be appropriate for the selected
 *                              algorithm and key:
 ...
 *          context, using a hash-and-sign verification algorithm.
 *
 * \note To perform a multi-part hash-and-sign signature verification
 *       algorithm, first use a multi-part hash operation to hash the message
 *       and then pass the resulting hash to psa_verify_hash().
...
/**
 * \brief Sign a hash or short message with a private key using a supplied
 *        context.
 *
 * Note that to perform a hash-and-sign signature algorithm, you must
 * first calculate the hash by calling psa_hash_setup(), psa_hash_update()
 ...
 *                              It must be an asymmetric key pair. The key must
 *                              allow the usage #PSA_KEY_USAGE_SIGN_HASH.
 * \param[in]  alg              A signature algorithm (PSA_ALG_XXX
 *                              value such that #PSA_ALG_IS_SIGN_HASH(\p alg)
 *                              is true), that is compatible with
 *                              the type of \p key.
 * \param[in]  hash             The hash or message to sign.
 ...
 *                             pair. The key must allow the usage
 *                             #PSA_KEY_USAGE_VERIFY_HASH.
 * \param[in] alg              A signature algorithm (PSA_ALG_XXX value
 *                             such that #PSA_ALG_IS_SIGN_HASH(\p alg) is true),
 *                             that is compatible with the type of \p key.
 * \param[in] hash             The hash or message whose signature is to be
 *                             verified.
 * \param[in] hash_length      Size of the \p hash buffer in bytes.
 * \param[in] context          The context to use for this signature.
