...
template_info['logo_file'] = 'Arm_logo_blue_RGB'
...
template_info['html_css_files'] = [
    ...
    ('https://fonts.googleapis.com/css2?family=Lato:ital,wght@0,400;0,700;1,400&display=swap', { 'rel': 'stylesheet' }),
    ('https://fonts.googleapis.com/css2?family=Inconsolata:wght400;700&display=swap', { 'rel': 'stylesheet' })
]
...
template_info['latex_fonts'] = [
        ...
        r'\usepackage[scaled=0.91]{inconsolata}'
        ]
template_info['latex_sphinxsetup'] = [
        ...
        'InnerLinkColor={rgb}{0,0.569,0.741}',
...
def make_doc_filename(info, id, title, version, status):
    if all((k in info for k in ('doc_id','quality','issue_no'))):
        return '-'.join([id.replace(' ',''), title, version])
    return None
